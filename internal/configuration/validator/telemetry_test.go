...
func TestValidateTelemetry(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			&schema.Configuration{Telemetry: schema.TelemetryConfig{
				Metrics: schema.TelemetryMetricsConfig{
					Address: schema.NewAddressFromNetworkValues(schema.AddressSchemeTCP, "0.0.0.0", schema.DefaultTelemetryConfig.Metrics.Address.Port()),
		...
		},
		{
			...
			nil,
			[]string{"telemetry: metrics: option 'address' must have a scheme of 'tcp', 'tcp4', 'tcp6', or 'unix' but it is configured as 'udp'"},
		},
	}
...
func TestValidateTelemetryShouldCorrectlyIdentifyValidAddressSchemes(t *testing.T) {
	testCases := []struct {
		...
		{schema.AddressSchemeUDP, "telemetry: metrics: option 'address' must have a scheme of 'tcp', 'tcp4', 'tcp6', or 'unix' but it is configured as 'udp'"},
		{schema.AddressSchemeUDP4, "telemetry: metrics: option 'address' must have a scheme of 'tcp', 'tcp4', 'tcp6', or 'unix' but it is configured as 'udp4'"},
		{schema.AddressSchemeUDP6, "telemetry: metrics: option 'address' must have a scheme of 'tcp', 'tcp4', 'tcp6', or 'unix' but it is configured as 'udp6'"},
		{schema.AddressSchemeUnix, ""},
		{"http", "telemetry: metrics: option 'address' must have a scheme of 'tcp', 'tcp4', 'tcp6', or 'unix' but it is configured as 'http'"},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.have, func(t *testing.T) {
			...
			case schema.AddressSchemeUnix:
				have.Telemetry.Metrics.Address = schema.NewAddressUnix("/path/to/authelia.sock")
			default:
				have.Telemetry.Metrics.Address = schema.NewAddressFromNetworkValues(tc.have, "", 9091)
			}
